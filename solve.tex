...
\end{align*}
Simplify to solve for \(\Delta \vv \lambda\) we get:
\[
...
since the vector $\vv{z-Rp}$ is needed elsewhere this has neglectable practical
benefit.
...
Q_z^{\T} \lambda}
\end{align*}
means we can solve for $\vv{u=L^{-\T}}\Delta z$ and then obtain $\vv \Delta z$
like so
\begin{align*}
